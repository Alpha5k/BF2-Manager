...
class StartButton extends InteractionHandler {
	constructor(ctx) {
		super(ctx, {
	...
	async run(interaction, {server_name}) {
		...
		return interaction.editReply(`${server_name} was started by <@${interaction.user.id}>`)
	}

	async parse(interaction) {
		if (interaction.customId != 'start') return this.none()
