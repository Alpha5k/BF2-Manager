...
class RestartButton extends InteractionHandler {
	constructor(ctx) {
		super(ctx, {
	...
	async run(interaction, {server_name}) {
		...
		return interaction.editReply(`${server_name} was restarted by <@${interaction.user.id}>`)
	}

	async parse(interaction) {
		if (interaction.customId != 'restart') return this.none()
