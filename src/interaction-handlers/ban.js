...
class BanButton extends InteractionHandler {
	constructor(ctx) {
		super(ctx, {
	...
	async run(interaction, {player_name, server_name}) {
		...
		var response = await server.banPlayer(player.Slot)
		if (response.Ok) {
			return interaction.reply(`${player.Name} was banned by <@${interaction.user.id}>`)
		}
	}

	async parse(interaction) {
		if (interaction.customId != 'ban') return this.none()
