...
class KickButton extends InteractionHandler {
	constructor(ctx) {
		super(ctx, {
	...
	async run(interaction, {player_name, server_name}) {
		...
		if (response.Ok) {
			return interaction.reply(`${player.Name} was kicked by <@${interaction.user.id}>`)
		}
	}

	async parse(interaction) {
		if (interaction.customId != 'kick') return this.none()
